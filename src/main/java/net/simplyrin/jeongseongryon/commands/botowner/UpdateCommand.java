...
import net.simplyrin.jeongseongryon.utils.Version;
...
public class UpdateCommand extends BaseCommand {
	...
	public String getDescription() {
		return "JeongSeongRyon のアップデート確認し、最新バージョンにアップデートします。";
	}
	...
	public void execute(JeongSeongRyon instance, PandaMessageEvent event, String[] args) {
		try {
			...
			if (Version.JENKINS_BUILDER_NUMBER.equalsIgnoreCase(buildNumber)) {
				event.reply("最新の JeongSeongRyon 🐼 を利用しています。\nhttps://ci.simplyrin.net/job/JeongSeongRyon/" + Version.JENKINS_BUILDER_NUMBER + "/");
			} else {
				event.reply("アップデートを確認しました。JeongSeongRyon 🐼 を更新しています...。\n"
						+ "現在のバージョン: **" + Version.JENKINS_BUILDER_NUMBER + "**\n"
						+ "最新のバージョン: **" + buildNumber + "**");

				File file = new File("JeongSeongRyon-1.0-jar-with-dependencies-v" + buildNumber + ".jar");
	...
	public boolean downloadJar(File file) {
		String url = this.jenkins + "/job/JeongSeongRyon/lastSuccessfulBuild/artifact/target/JeongSeongRyon-1.0-jar-with-dependencies.jar";
		try {
	...
	public String getStableBuildNumber() {
		String url = this.jenkins + "/job/JeongSeongRyon/lastStableBuild/buildNumber";
		try {
			HttpsURLConnection connection = (HttpsURLConnection) new URL(url).openConnection();
			connection.addRequestProperty("user-agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/88.0.4324.150 Safari/537.36");
			connection.connect();
